...
static Double gf_dash_get_max_available_speed(GF_DashClient *dash, GF_DASH_Group *group, GF_MPD_Representation *rep)
{
	...
	if (rep->playback.decode_only_rap) 
		...
		//if framerate is not in mdp, support that it is 25
		u32 framerate = rep->framerate ? rep->framerate->num : 25;
		//FIXME: not correct with audio or video which has more than 1 I-frame per second
	...
	return max_available_speed / 2; //for test - demo
} 
	...
	for (k=0; k<gf_list_count(group->adaptation_set->representations); k++) {
		...
		if (dl_rate >= arep->bandwidth) {
			if (force_below_resolution && !dash->disable_speed_adaptation) {
				GF_LOG(GF_LOG_DEBUG, GF_LOG_DASH, ("[DASH] Speed adaptation\n"));
				...
				GF_LOG(GF_LOG_DEBUG, GF_LOG_DASH, ("[DASH] Bitrate adaptation\n"));
